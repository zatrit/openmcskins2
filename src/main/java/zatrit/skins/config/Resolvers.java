  ...
  public static @Nullable Resolver resolverFromEntry(@NotNull HostEntry entry) {
    ...
    try {
      return switch (entry.getType()) {
        ...
        case FALLBACK ->
            new FallbackResolver(config, MinecraftClient.getInstance().getSessionService());
        ...
        case NAMED_HTTP, OPTIFINE, VALHALLA, DIRECT -> {
          ...
          yield switch (entry.getType()) {
            ...
            case DIRECT -> {
              val types =
                  ((List<String>) props.get("types")).stream().map(TextureType::valueOf).toList();
        ...
        case LOCAL -> {
          ...
          val directory =
              Path.of(
                  str(directoryPattern)
                      .arg("configDir", instance.getConfigDir())
                      .arg("gameDir", instance.getGameDir())
                      .fmt());
