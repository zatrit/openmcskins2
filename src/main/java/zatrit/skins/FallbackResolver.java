...
import zatrit.skins.util.TextureTypeUtil;
...
public class FallbackResolver implements Resolver {
  ...
  @Override
  public @NotNull PlayerTextures resolve(Profile profile) throws VerifyException {
    ...
    for (val type : MinecraftProfileTexture.Type.values()) {
      ...
      val metadata =
          new Metadata(
              Boolean.parseBoolean(texture.getMetadata("animated")), texture.getMetadata("model"));

      newTextures.put(
          TextureTypeUtil.fromAuthlibType(type), new URLTexture(texture.getUrl(), metadata));
    }

    return new CachedPlayerTextures<>(newTextures, config.getLayers(), config.getCache());
  }
}
