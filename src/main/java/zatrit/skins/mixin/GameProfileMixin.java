    ...
    return CompletableFuture.supplyAsync(
        new Supplier<Profile>() {
          ...
          public Profile get() {
            ...
            val map = SkinsClient.getSkinlibConfig().getGson().fromJson(reader, Map.class);

            val id =
                String.valueOf(map.get("id"))
                    .replaceAll("(\\w{8})(\\w{4})(\\w{4})(\\w{4})(\\w{12})", "$1-$2-$3-$4-$5");

            val profile = new GameProfile(UUID.fromString(id), GameProfileMixin.this.getName());
            profile.getProperties().putAll(GameProfileMixin.this.getProperties());
