...
public class TextUtil {
  private static final List<?> SKIP_ELEMENTS =
      Lists.newArrayList(null, Collections.emptyMap(), Collections.emptyList());

  /** Converts {@link Map} into a nicely formatted {@link Text}. */
  @SuppressWarnings("unchecked")
  public static void mapToText(@NotNull MutableText text, @NotNull Map<String, ?> map) {
    val specialStyle =
        (UnaryOperator<net.minecraft.text.Style>) style -> style.withFormatting(Formatting.GRAY);
    ...
    for (val entry : map.entrySet()) {
      ...
      text.append(
          Text.literal(entry.getKey()).styled(style -> style.withFormatting(Formatting.RESET)));
      text.append(Text.literal(" = ").styled(specialStyle));
      ...
      if (value instanceof Number numberValue) {
        ...
        var mutableText = Text.literal(stringValue).formatted(Formatting.GREEN);

        if (isURL(stringValue)) {
          val clickAction = new ClickEvent(ClickEvent.Action.OPEN_URL, stringValue);
          mutableText =
              mutableText.styled(
                  s -> s.withFormatting(Formatting.UNDERLINE).withClickEvent(clickAction));
        }
